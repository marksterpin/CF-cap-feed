...
const UA = "CapNavigator-CDR-Harvester/1.2 (+https://github.com/)";
...
async function cdrGet(url, xv) {
  // Try from a high x-v downwards. With x-min-v:1 the holder answers with the highest
  // version it supports in [1..x-v], so newer holders that retired old versions still work.
  const versions = Array.from({ length: xv }, (_, i) => xv - i);
...
async function discoverHolders() {
  ...
  let holders = arr
    ...
    // drop CDR sandbox / mock / test brands — they publish fake products
    .filter(h => !/\b(pm production|test|mock|sandbox|demo)\b/i.test(h.name));
...
async function listProducts(base, category) {
  ...
  for (let guard = 0; url && guard < 15; guard++) {
    const j = await cdrGet(url, 6);   // start high — majors have retired v1-3
...
async function productDetail(base, id) {
  const j = await cdrGet(base + "/cds-au/v1/banking/products/" + encodeURIComponent(id), 7); // start high — majors have retired old versions
  return j.data || {};
}

// CDR PRD product + detail -> CapNavigator feed schema (null if no comparable rate).
function mapProduct(brandName, p, d, category) {
  // Headline rates only: DISCOUNT / BUNDLE / PENALTY / INTRODUCTORY entries are
  // adjustments or teasers, and 0% is a placeholder — picking those corrupts rankings.
  const HEADLINE = ["VARIABLE", "FIXED", "FLOATING", "MARKET_LINKED"];
  const lr = (d.lendingRates || [])
    .map(r => ({ t: (r.lendingRateType || "").toUpperCase(), rate: num(r.rate) }))
    .filter(r => r.rate != null && r.rate > 0 && HEADLINE.includes(r.t));
  if (!lr.length) return null; // no usable headline rate — pricing on application
  ...
  // revolving facilities are typed by name as well as category — holders often list
  // overdrafts and lines of credit under BUSINESS_LOANS
  const isLoc = /line of credit|revolving/.test(text);
  const isOd = /overdraft/.test(text);
  const od = category === "OVERDRAFTS" || isLoc || isOd;
  const type = od ? (isLoc ? "line_of_credit" : "overdraft")
...
function selfTest() {
  ...
  const locByName = mapProduct("Test Bank", { name: "Commercial Revolving Line of Credit", productId: "loc-2" },
    { lendingRates: [{ lendingRateType: "VARIABLE", rate: "0.1175" }] }, "BUSINESS_LOANS");
  const zeroRate = mapProduct("Test Bank", { name: "Placeholder Loan" },
    { lendingRates: [{ lendingRateType: "VARIABLE", rate: "0.00" }] }, "BUSINESS_LOANS");
  const discountOnly = mapProduct("Test Bank", { name: "Package Discount Loan" },
    { lendingRates: [{ lendingRateType: "DISCOUNT", rate: "0.0150" }] }, "BUSINESS_LOANS");
  console.log("name-typed LOC →", locByName && locByName.type, "| zero-rate →", zeroRate, "| discount-only →", discountOnly, "(both expected null)");
  ...
  if (!m || m.rate !== 8.99 || m.feeFlat !== 500 || m.feeMo !== 20 || m.min !== 10000 || m.max !== 750000 || m.type !== "secured_term"
      ...
      || !loc || loc.type !== "line_of_credit"
      || !locByName || locByName.type !== "line_of_credit" || locByName.io !== true
      || zeroRate !== null || discountOnly !== null || none !== null) {
